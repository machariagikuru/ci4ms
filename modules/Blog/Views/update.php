        ...
        <div class="row mb-2">
            ...
            <div class="col-sm-6">
                <ol class="breadcrumb float-sm-right">
                    <a href="<?= route_to('blogs', 1) ?>"
                       class="btn btn-outline-info"><?= lang('Backend.backToList') ?></a>
                </ol>
            </div>
        </div>
    </div><!-- /.container-fluid -->
...
<section class="content">
    ...
    <div class="card card-outline card-shl">
        ...
        <div class="card-body">
            ...
            <form action="<?= route_to('blogUpdate', $infos->id) ?>" class="form-row" method="post">
                <?= csrf_field() ?>
                        ...
                        <div class="btn-group btn-group-toggle w-100" data-toggle="buttons">
                            ...
                            <label class="btn btn-outline-secondary active">
                                <input type="radio" name="isActive" id="option2"
                                       autocomplete="off" <?= ((bool)$infos->isActive === true) ? 'checked' : '' ?>
                                       value="1">
                    ...
                    <div class="col-md-12 form-group">
                        ...
                        <div class="input-group">
                            ...
                            <input type="text" class="form-control" id="datemask" name="created_at"
                                   value="<?= date('d.m.Y H:i:s', strtotime($infos->created_at)) ?>" required>
                        </div>
                    </div>
                    <div class="col-md-12 form-group">
                        ...
                        <select name="categories[]" id="" class="form-control select2bs4" multiple="multiple"
                            ...
                            if (!empty($infos->categories)):
                                ...
                                foreach ($selected as $select):?>
                                    <option value="<?= $select->id ?>" selected><?= $select->title ?></option>
                                <?php endforeach;
                            endif;
...
<script>
    $.post('<?=route_to('tagify')?>', {"<?=csrf_token()?>": "<?=csrf_hash()?>"}, 'json').done(function (data) {
        tags(data);
    });
