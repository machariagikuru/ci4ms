...
class CommonTagsLibrary
{
    ...
    {
        if ($isUpdate === true) {
            $this->commonModel->remove('tags_pivot', ['piv_id' => $insertedID, 'tagType' => $type]);
        }
        ...
        if ($jsons === null) {
            // Optionally log or throw an exception, or just return early
            return;
        }

        // Ensure we have an iterable (array or object)
        ...
        foreach ($jsons as $item) {
            if (!empty($item->id)) {
                ...
                $tag = $this->commonModel->selectOne('tags', ['tag' => $item->value]);
                if (empty($tag) || ($tag && $item->value != $tag->tag)) {
                    $max_url_increment = 10000;
                    $link = null;
                    if ($this->commonModel->isHave($table, ['seflink' => seflink($item->value, ['lowercase'])]) === 0) {
